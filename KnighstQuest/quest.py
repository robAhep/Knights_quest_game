...
def setup_game():
    ...
    game_over = False
    player_won = False
    player = Actor("mario", anchor=("left","top"))
    keys_to_collect = []
    guards = []
    for y in range(GRID_HEIGHT):
        for x in range(GRID_WIDTH):
            ...
            if square =="P":
                ...
                key = Actor("gold_key", anchor=("left", "top"), \
                            pos=screen_coords(x, y))
                keys_to_collect.append(key)
            elif square == "G":
                guard = Actor("turtle", anchor=("left", "top"), \
                              pos=screen_coords(x, y))
                guards.append(guard)
def draw_background():
...
def draw_scenery():
    for y in range(GRID_HEIGHT):
        for x in range(GRID_WIDTH):
            square = MAP[y][x]
            if square == "W":
                screen.blit("block", screen_coords(x, y))
            elif square =="D" and len(keys_to_collect) > 0:
                screen.blit("castle", screen_coords(x, y))
...
def move_guard(guard):
    ...
    # Function to check if a position is occupied by another guard
    def is_position_occupied(x, y):
        for other_guard in guards:
            if other_guard != guard and grid_coords(other_guard) == (x, y):
                return True
        return False

    # Determine the new position
    new_guard_x, new_guard_y = guard_x, guard_y
    if player_x > guard_x and MAP[guard_y][guard_x + 1] != "W" and not is_position_occupied(guard_x + 1, guard_y):
        new_guard_x += 1
    elif player_x < guard_x and MAP[guard_y][guard_x - 1] != "W" and not is_position_occupied(guard_x - 1, guard_y):
        new_guard_x -= 1
    elif player_y > guard_y and MAP[guard_y + 1][guard_x] != "W" and not is_position_occupied(guard_x, guard_y + 1):
        new_guard_y += 1
    elif player_y < guard_y and MAP[guard_y - 1][guard_x] != "W" and not is_position_occupied(guard_x, guard_y - 1):
        new_guard_y -= 1

    # Move the guard if the new position is valid
    if (new_guard_x, new_guard_y) != (guard_x, guard_y):
        animate(guard, pos=screen_coords(new_guard_x, new_guard_y), duration=GUARD_MOVE_INTERVAL)

    # Check if the guard has caught the player
    if new_guard_x == player_x and new_guard_y == player_y:
        game_over = True
