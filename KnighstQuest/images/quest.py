...
def setup_game():
    global game_over, player_won, keys_to_collect
    game_over = False
    player_won = False
...
def move_guard(guard):
    ...
    if player_x > guard_x and MAP[guard_y][guard_x + 1] != "W":
        ...
        guard_x -= 1
    elif player_y > guard_x and MAP[guard_y][guard_x + 1] != "W":
        guard_y += 1
    elif player_y < guard_y and MAP[guard_y][guard_x - 1] != "w":
        guard_y -= 1
    animate(guard,pos=screen_coords(guard_x, guard_y), \
            duration=GUARD_MOVE_INTERVAL)
    if guard_x == player_x and guard_y:
        game_over = True
